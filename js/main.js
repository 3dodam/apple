...
(function(){
  ...
  let prevScrollHeight=0; //현재 활성화된 section 이전의 높이 값
  let currentScene=0; //현재 활성화된(화면에 보이는) section
  let enterNewScene=false; //새로운section에 진입했을때 알려주는 역할

  function setCanvasImages(){
      //canvas에 이미지를 기리기위한 함수
      let imgEle;
      for(let i=0; i<sceneInfo[0].values.videoImageCount; i++){
          //sceneInfo.js에서 videoImageCount가져옴
          //imgEle=document.createElement('img');
          imgEle=new Image(); //이미지 태그만들기
          imgEle.src=`./video/001/img_${1 + i}.jpg`;
          sceneInfo[0].objs.videoImages.push(imgEle) //배열에 값을 넣을때 push 사용
      }
      ...
      for(let i=0; i<sceneInfo[2].values.videoImageCount; i++){
          //sceneInfo.js에서 videoImageCount가져옴
          //imgEle=document.createElement('img');
          imgEle2=new Image(); //이미지 태그만들기
          imgEle2.src=`./video/002/img_${301 + i}.jpg`;
          sceneInfo[2].objs.videoImages.push(imgEle2) //배열에 값을 넣을때 push 사용
      }
  }
  setCanvasImages();
  ...
  setLayout();
  sceneInfo[0].objs.context
  objs.context.drawImage(sceneInfo[0].objs.videoImages[0], 0, 0, 1920, 1080)
